...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i, layer in enumerate(style_layers):
        # Get features for the current image at the specified layer
        current_features = feats[layer]
        # Compute the Gram matrix for the current image
        current_gram = gram_matrix(current_features)
        # Get the Gram matrix of the style image at the specified layer
        style_gram = style_targets[i]
        # Compute the style loss for this layer
        layer_loss = style_weights[i] * torch.sum((current_gram - style_gram) ** 2)
...
def guided_gram_matrix(features, masks, normalize=True):
  ...
  guided_gram = torch.zeros(N, R, C, C).to(features.device)
  
  for n in range(N):
      for r in range(R):
          # Apply the mask to the feature map
          masked_features = features[n, r] * masks[n, r].unsqueeze(0)
          # Reshape masked features to (C, H * W)
          masked_features = masked_features.view(C, -1)
          # Compute the Gram matrix for the masked features
          gram = torch.mm(masked_features, masked_features.t())
          if normalize:
              gram /= (C * H * W)
          guided_gram[n, r] = gram
...
def guided_style_loss(feats, style_layers, style_targets, style_weights, content_masks):
    ...
    loss = 0.0
    for i, layer in enumerate(style_layers):
        # Get features and masks for the current layer
        current_features = feats[layer]
        current_masks = content_masks[layer]
        
        # Compute the guided Gram matrix for the current image
        current_gram = guided_gram_matrix(current_features, current_masks)
        
        # Get the guided Gram matrix of the style image at the specified layer
        style_gram = style_targets[i]
        
        # Compute the style loss for this layer and accumulate
        layer_loss = style_weights[i] * torch.sum((current_gram - style_gram) ** 2)
        loss += layer_loss
