...
def discriminator_loss(logits_real, logits_fake):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    true_labels = torch.ones(logits_real.size(), device=logits_real.device)
    # 가짜 데이터에 대한 거짓 레이블
    fake_labels = torch.zeros(logits_fake.size(), device=logits_real.device)
...
def generator_loss(logits_fake):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    fake_labels = torch.ones(logits_fake.size(), device=logits_fake.device)
    m = nn.Sigmoid()
    loss = nn.BCELoss()
    fake_loss = loss(m(logits_fake), fake_labels)
    #print(true_loss,fake_loss)
    loss = fake_loss 
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
